...
class PolicyNetwork(nn.Module):
    """Policy network that's used in the first experiment"""
    def __init__(self, state_size, action_dim: int, lr: float, 
                 hidden_dim: int = 5, batch_size: int = 1):
        super().__init__()
...
class VanillaPGExperiment:
    ...
    def pg_cnn_transfer(self):
        ...
        for episode_idx in range(self.num_episodes):
            ...
            returns = rl_memory.tools.discount_rewards(rewards, self.disc_factor)
            baselines = np.zeros(returns.shape)
            advantages = returns - baselines
            if len(log_probs) != len(advantages):
                print()
...
def train(env: Env = env, agent: Agent = james_bond, state: State = state,
    ...
    action_dim = len(env.action_space)
    actor = PolicyNetwork(state_dim, action_dim, lr)
    ...
    for episode in range(num_episodes):
        ...
        while not d:
            ...
            action_distribution = actor.action_distribution(state.observation.flatten())
